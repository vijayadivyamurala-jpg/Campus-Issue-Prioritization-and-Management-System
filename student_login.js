...
document.getElementById("loginForm").addEventListener("submit", async function(e){
    ...
    if(!pattern.test(email)){
        return showError("Enter a valid student email.");
    }

    const result = await CampusApp.loginUser("student", email, password);
    if(!result.ok){
...
document.getElementById("resetForm").addEventListener("submit", async function (e) {
    ...
    if (!email) {
        return showResetMessage("Enter your registered student email.");
    }
    if (!pattern.test(email)) {
        return showResetMessage("Enter a valid student email.");
    }
